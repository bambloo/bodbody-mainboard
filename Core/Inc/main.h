/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2026 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32h7xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define DEFAULT_APP_STACK_SIZE 0x0800
#define DEFAULT_MEMORY_POOL_SIZE 0x1000
#define CODE_GENERATED 1
#define ETH_RST_Pin GPIO_PIN_15
#define ETH_RST_GPIO_Port GPIOA
#define EN_OSC_12M_Pin GPIO_PIN_4
#define EN_OSC_12M_GPIO_Port GPIOB
#define CH132_RESET_Pin GPIO_PIN_3
#define CH132_RESET_GPIO_Port GPIOE
#define ESP_DATAREADY_Pin GPIO_PIN_9
#define ESP_DATAREADY_GPIO_Port GPIOB
#define TF_DETECT_Pin GPIO_PIN_9
#define TF_DETECT_GPIO_Port GPIOA
#define ESP_EN_Pin GPIO_PIN_13
#define ESP_EN_GPIO_Port GPIOC
#define ESP_HANDSHAKE_Pin GPIO_PIN_8
#define ESP_HANDSHAKE_GPIO_Port GPIOI
#define MCO_24M_Pin GPIO_PIN_8
#define MCO_24M_GPIO_Port GPIOA
#define LCD_UD_Pin GPIO_PIN_7
#define LCD_UD_GPIO_Port GPIOC
#define LCD_LR_Pin GPIO_PIN_6
#define LCD_LR_GPIO_Port GPIOC
#define RK628_INT_Pin GPIO_PIN_3
#define RK628_INT_GPIO_Port GPIOG
#define RK628_PWR_EN_Pin GPIO_PIN_2
#define RK628_PWR_EN_GPIO_Port GPIOG
#define I2C_SCL2_Pin GPIO_PIN_6
#define I2C_SCL2_GPIO_Port GPIOA
#define LED_EN_Pin GPIO_PIN_0
#define LED_EN_GPIO_Port GPIOA
#define TP_RESET_Pin GPIO_PIN_6
#define TP_RESET_GPIO_Port GPIOH
#define I2C_SDA2_Pin GPIO_PIN_4
#define I2C_SDA2_GPIO_Port GPIOA
#define LCD_RESET_Pin GPIO_PIN_1
#define LCD_RESET_GPIO_Port GPIOG
#define TP_INT_Pin GPIO_PIN_7
#define TP_INT_GPIO_Port GPIOH
#define TP_INT_EXTI_IRQn EXTI9_5_IRQn

/* USER CODE BEGIN Private defines */

/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
