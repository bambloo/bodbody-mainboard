...
static VOID usbx_thread_entry(ULONG thread_input);
...
UINT MX_USBX_Host_Init(VOID *memory_ptr)
{
  ...
  if (tx_thread_create(&ux_host_app_thread, UX_HOST_APP_THREAD_NAME, usbx_thread_entry,
...
/**
  * @brief  Function implementing usbx_thread_entry.
  * @param  thread_input: User thread input parameter.
  * @retval none
  */
static VOID usbx_thread_entry(ULONG thread_input)
{
  /* USER CODE BEGIN usbx_thread_entry */
  MX_USBX_Host_Stack_Init();
  HAL_HCD_Start(&hhcd_USB_OTG_HS);
  /* USER CODE END usbx_thread_entry */
}
