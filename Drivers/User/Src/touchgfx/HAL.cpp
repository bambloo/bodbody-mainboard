...
void HAL::blitCopy(const uint16_t *pSrc, uint16_t x, uint16_t y, uint16_t width,
                   uint16_t height, uint16_t srcWidth, uint8_t alpha,
                   bool hasTransparentPixels, bool replaceBgAlpha) {
  auto format = lcd().framebufferFormat();
  return blitCopy(pSrc, nullptr, x, y, width, height, srcWidth, alpha,
                  hasTransparentPixels, FRAME_BUFFER_WIDTH, format, format,
                  replaceBgAlpha);
}
